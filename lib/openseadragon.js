...
const toChannel = (channel_in) => {
  const { idx, success, channel, imageSource } = channel_in;
...
const newContext = (state, opts, reset) => {
  ...
  idxArray.map(idx => {
    ...
    const success = state.trackLayer(idx);
    const channel_in = { idx, success, channel, imageSource };
    viewer.addTiledImage(toChannel(channel_in));
  });
